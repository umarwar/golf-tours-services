...
class PgatourCompletedSpider(scrapy.Spider):
    name = "pgatour_completed_spider"
    start_urls = ["https://www.pgatour.com/schedule"]
    custom_settings = {
        "ROBOTSTXT_OBEY": False,
        "CONCURRENT_REQUESTS": 4,
        "FEEDS": {
            "completed_tournaments_output/01_pgatour_completed_schedule.csv": {
                "format": "csv",
                "overwrite": True,
                "encoding": "utf-8-sig",
            }
    ...
    def parse(self, response):
        ...
        try:
            ...
            # Extract completed tournaments
            completed_tournaments = (
                schedule_query.get("state", {}).get("data", {}).get("completed", [])
            )

            self.logger.info(
                f"Found {len(completed_tournaments)} completed tournament months"
            )

            # Process each month's tournaments
            for month_data in completed_tournaments:
                ...
                for tournament in tournaments:
                    try:
                        ...
                        tournament_status = tournament.get("tournamentStatus", "")
